...
export async function processTokenAdmins(network, upsertedTokens) {
  // Filter upserted tokens with payoutRecipientNotDefaultAdmin not null
  const tokensWithPayoutRecipient = upsertedTokens.filter(
    token => token.payoutRecipientNotDefaultAdmin != null
  );

  if (tokensWithPayoutRecipient.length === 0) {
    return;
  }

  // For each, get distributors (getDistributors returns null if not a split contract)
  const distributorsInserts = [];
  for (const token of tokensWithPayoutRecipient) {
    try {
      const payoutRecipient =
        token.payoutRecipientNotDefaultAdmin.toLowerCase();
      const chainId = token.chainId;

      // Get distributors - returns null if not a split contract
      const distributors = await getDistributors(payoutRecipient, chainId);
      ...
      if (distributors && distributors.length > 0) {
        for (const distributor of distributors) {
          ...
          distributorsInserts.push({
            token_id: token.id,
            artist_address: distributor.address.toLowerCase(),
          });
        }
      }
    } catch (err) {
