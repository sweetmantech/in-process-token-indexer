  ...
  for (let i = 0; i < momentEvents.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (validMoments.length > 0) {
        ...
        const tokensForUpsert = validMoments.map(({ payoutRecipientNotDefaultAdmin, ...token }) => token);
        await upsertTokens(tokensForUpsert);

        const momentsWith0xSplits = validMoments.filter(moment => moment.payoutRecipientNotDefaultAdmin);
        if (momentsWith0xSplits.length > 0) {
          const isSplit = await isSplitContract(momentsWith0xSplits[0].payoutRecipientNotDefaultAdmin, network);
          // const momentsWith0xSplitsAddresses = momentsWith0xSplits.map(moment => moment.address);
          // await ensureArtists(momentsWith0xSplitsAddresses);
          // await upsertTokens(momentsWith0xSplits);

          console.log('ziad here', momentsWith0xSplits.map((moment) => ({
