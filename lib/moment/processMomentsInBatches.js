...
import { processTokenAdmins } from './processTokenAdmins.js';
  ...
  for (let i = 0; i < momentEvents.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (validMoments.length > 0) {
        ...
        // Process token admins for tokens with split contract payout recipients
        await processTokenAdmins(network, upsertedTokens);
