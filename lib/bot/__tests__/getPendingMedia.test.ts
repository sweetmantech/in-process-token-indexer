...
describe('getPendingMedia', () => {
  ...
  it('should return PendingMedia even for large files (download errors happen downstream)', async () => {
    // Files > 20MB cannot be fetched via getFile, but getPendingMedia does not call
    // getFile — it only reconstructs PendingMedia from the encoded fileId. The actual
    // download (and its error) happens later in getBlob/processVideo.
    const mediaInfo = `MEDIA:video:${mockFileId}`;
    const encodedMediaInfo = encodeMediaInfo(mediaInfo);
    const titleRequestText = `📝 Please send the **title** for your moment:\n${encodedMediaInfo}`;
  ...
  it('should correctly reconstruct photo object with file_id', async () => {
    const mediaInfo = `MEDIA:photo:${mockFileId}`;
    const encodedMediaInfo = encodeMediaInfo(mediaInfo);
    const titleRequestText = `📝 Please send the **title** for your moment:\n${encodedMediaInfo}`;
  ...
  it('should correctly reconstruct video object with file_id', async () => {
    const mediaInfo = `MEDIA:video:${mockFileId}`;
    const encodedMediaInfo = encodeMediaInfo(mediaInfo);
    const titleRequestText = `📝 Please send the **title** for your moment:\n${encodedMediaInfo}`;
