 ...
 * Each object should have: { token_id: string, artist_address: string }
...
export async function upsertTokenAdmins(tokenAdmins) {
  ...
  // Remove duplicates based on conflict columns (token_id and artist_address)
  const uniqueTokenAdmins = tokenAdmins.filter(
    (tokenAdmin, index, self) =>
      index ===
      self.findIndex(
        ta =>
          ta.token_id === tokenAdmin.token_id &&
