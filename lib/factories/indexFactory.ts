 ...
 */
export function createIndexFunction<T>({
  queryFn,
  processBatchFn,
  ...
  return async function index(): Promise<T[]> {
    ...
    const maxTimestampSupabase = await selectMaxTimestampFn();
    const minTimestampEnvio = toChainTimestamp(
      maxTimestampSupabase ?? new Date(0).getTime()
    );

    while (hasNextPage) {
      const { entities, pageInfo } = await queryFn({
        limit,
        offset,
      ...
      if (entities.length > 0) {
        console.log(
          `💻 Processing ${allEntities.length} ~ ${allEntities.length + entities.length} ${indexName}`
        );
      }

      // ℹ️ Process fetched entities for this page (batch upserts handled in processBatchFn)
      await processBatchFn(entities);
