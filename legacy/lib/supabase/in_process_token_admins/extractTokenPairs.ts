...
export function extractTokenPairs(adminEvents: AdminEvent[]): TokenPair[] {
  ...
  const validEvents = adminEvents.filter(
    event => event.tokenContract && event.tokenContract.trim().length > 0
  );
  ...
  if (validEvents.length < adminEvents.length) {
    ...
    console.debug(
      `extractTokenPairs: Skipped ${skippedCount} admin event(s) without tokenContract`
    );
  }
