...
export async function processAdminsInBatches(network, adminEvents) {
    ...
    logForBaseOnly(network, `${network} - Processing ${adminEvents.length} admin permission events in ${totalBatches} batches of ${BATCH_SIZE}`);
    for (let i = 0; i < adminEvents.length; i += BATCH_SIZE) {
        const batch = adminEvents.slice(i, i + BATCH_SIZE);
        const batchNumber = Math.floor(i / BATCH_SIZE) + 1;
        try {
            logForBaseOnly(network, `${network} - Processing batch ${batchNumber}/${totalBatches} (${batch.length} admin events)`);
            ...
            if (mappedAdmins.length > 0) {
                ...
                logForBaseOnly(network, `${network} - Batch ${batchNumber}: No valid admin events found (tokens may not exist in in_process_tokens table)`);
            }
    ...
    logForBaseOnly(network, `${network} - Completed processing: ${totalProcessed} admin permission events indexed`);
}
//# sourceMappingURL=processAdminsInBatches.js.map