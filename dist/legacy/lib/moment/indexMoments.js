...
const getMaxBlockTimestamp_1 = require("./getMaxBlockTimestamp");
/**
 * Indexes moment events from the GRPC endpoint using the factory pattern
 */
async function indexMoments() {
    const momentsIndexer = new IndexerFactory_1.IndexerFactory({
        name: 'moments',
    ...
    `,
        ...
        processFunction: async (network, events) => {
            const { processMomentsInBatches } = await Promise.resolve().then(() => __importStar(require('./processMomentsInBatches.js')));
            return await processMomentsInBatches(network, events);
        },
    });
    return await momentsIndexer.start();
}
exports.default = indexMoments;
